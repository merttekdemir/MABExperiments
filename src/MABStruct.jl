...
#---------------------------------------------------------------------------------------------------
"""
set_instance!(bandit::MABStruct, bandit_other::MABStruct)

Sets the parameters of a game instance equal to another. This is used for efficiency reasons in MABExperiments.
The game run on the experiments has alwasy the same memory allocated, we store the results by setting other game instances 
to the one that has run last. See MABExperiments.jl for better understanding.

...
### Arguments 
- `bandit::MABStruct`: Game to be redefined.
- `bandit_other::MABStruct`: Reference game.
"""
